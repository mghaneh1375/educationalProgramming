...
class ReportHandler extends BaseController {
    ...
    public function lessons() {
        $degrees = Degree::all();
        return View::make('lessons', array('mode' => 'see', 'selectedDegree' => '', 'degrees' => $degrees, 'msg' => ''));
    }

    public function opOnLessons() {
        $selectedDegree = '';
        if(isset($_POST["addNewLesson"])) {
            $degreeId = makeValidInput(Input::get('selectedDegree'));
            $lesson = makeValidInput(Input::get('lesson'));
            $selectedDegree = $degreeId;
            if(empty($lesson))
                $msg = 'لطفا نامی برای درس مورد نظر انتخاب نمایید';
            else {
                try {
                    $newLesson = new Lesson();
                    $newLesson->name = $lesson;
                    $newLesson->did = $degreeId;
                    $newLesson->save();
                    $msg = 'درس مورد نظر به پایه ی تحصیلی مورد نظر اضافه شد';
                } catch (Exception $e) {
                    $msg = 'درس مورد نظر در پایه ی تحصیلی مورد نظر وجود دارد';
                }
            }
        }
        
        else if(isset($_POST['deleteSelected'])) {
            $lid = makeValidInput(Input::get('deleteSelected'));
            Lesson::destroy($lid);
            $msg = 'درس مورد نظر به درستی از سیستم حذف شد';
        }
        else if(isset($_POST['editSelected'])) {
            $degrees = Degree::all();
            $selectedLesson = makeValidInput(Input::get('editSelected'));
            $selectedDegree = makeValidInput(Input::get('selectedDegree'));
            $lessonName = Lesson::where('id', '=', $selectedLesson)->select('lesson.name')->get()[0]->name;
            return View::make('lessons', array('mode' => 'edit', 'name' => $lessonName, 'selectedDegree' => $selectedDegree,
                'selectedLesson' => $selectedLesson, 'degrees' => $degrees, 'msg' => ''));
        }
        else if(isset($_POST['doEditLesson'])) {
            $newDid = makeValidInput(Input::get('newSelectedDegree'));
            $lId = makeValidInput(Input::get('selectedLesson'));
            $newLessonName = makeValidInput(Input::get('newLessonName'));
            try{
                ...
                $msg = 'درسی با همین نام در پایه ی تحصیلی مورد نظر وجود دارد';
                $lesson = Lesson::where('id', '=', $lId)->get()[0];
                $lessonName = $lesson->name;
                $newDid = $lesson->did;
                $degrees = Degree::all();
                return View::make('lessons', array('mode' => 'edit', 'name' => $lessonName, 'selectedDegree' => $newDid,
                    'selectedLesson' => $lId, 'degrees' => $degrees, 'msg' => $msg));
            }
        }
        else
        ...
        $degrees = Degree::all();
        return View::make('lessons', array('mode' => 'see', 'selectedDegree' => $selectedDegree, 'degrees' => $degrees, 'msg' => $msg));
    }
